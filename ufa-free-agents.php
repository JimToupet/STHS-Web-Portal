...
<article>
    ...
    <section>
        ...
        <ul>
              <li id="selected"><a href="ufa-free-agents.php"><?php echo $l_tab1; ?></a></li>
        ...
        <div id="tab-content">
		   <?php 
				$query_GetSkaters = sprintf("SELECT P.* FROM players as P WHERE Not Exists (SELECT 1 FROM playerscontractoffers AS O WHERE O.Player=P.Number AND O.PlayerType <> 'goalie') AND Not Exists (SELECT 1 FROM playersextensionoffers AS E WHERE E.Player=P.Number AND E.PlayerType <> 'goalie') AND (P.Contract=0 OR P.Team = 0) AND P.Retired=0 ORDER BY Overall desc" );
            ...
            <table  cellspacing="0" border="0" width="100%" class="tablesorter">
                   ...
                   echo '<div style="display:block; float:left; width:20px; text-align:center; vertical-align:middle">';
                    ...
                    <td align="right" nowrap="nowrap">
                        <?php if ($_SESSION['U_ID'] <> ""){ ?>
							<a href="offerSheet.php?player=<?php echo $row_GetSkaters['Number']; ?>&type=player" ><?php echo $l_Send;?></a>
						<?php } ?></td>
                  </tr>
                ...
                <?php } ?> 
                 ...
                 <?php 
					$query_GetGoalies = sprintf("SELECT G.* FROM goalies as G WHERE Not Exists (SELECT 1 FROM playerscontractoffers AS O WHERE O.Player=G.Number AND O.PlayerType <> 'player') AND Not Exists (SELECT 1 FROM playersextensionoffers AS E WHERE E.Player=G.Number AND E.PlayerType = 'goalie') AND (G.Contract=0 OR G.Team = 0) AND G.Retired=0 ORDER BY Overall desc");
                ...
                <?php
                  ...
                  <tr>
                    ...
                    <td align="right" nowrap="nowrap">
                        <?php if ($_SESSION['U_ID'] <> ""){ ?>
							<a href="offerSheet.php?player=<?php echo $row_GetGoalies['Number']; ?>&type=goalie" ><?php echo $l_Send;?></a>
						<?php } ?></td>
                  </tr>
                  <?php 
