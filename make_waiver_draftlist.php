...
$query_GetRoster = sprintf("SELECT Overall, Name, Number, PO, Age FROM players WHERE (Contract = 0 OR Team='' OR Team IS NULL) AND Retired=0 AND Suspension<99 AND Name NOT IN (SELECT Name FROM waiver_draftpicks WHERE Draft_ID=%s) %s UNION ALL SELECT Overall, Name, PO, Number, Age FROM goalies WHERE (Contract = 0 OR Team='' OR Team IS NULL) AND Retired=0 AND Suspension<99 AND Name NOT IN (SELECT Name FROM waiver_draftpicks WHERE Draft_ID=%s) %s ORDER BY Overall desc , Name asc", $GetDraftID, $extra_SQL, $GetDraftID, $extra_SQL);
...
<tr>
	<td>
        ...
        <?php echo $row_GetDraftList['DraftList'];
        	do { 
