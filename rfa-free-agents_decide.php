    ...
    <h1><?php echo $l_nav_restricted_free_agents;?></h1> 
       <br /><br clear="all" />
        ...
        <ul>
              ...
              <li id="selected"><a href="rfa-free-agents_decide.php"><?php echo $l_tab3; ?></a></li>
        ...
        <div id="tab-content">
            ...
            <?php 
				$query_GetSkaters = sprintf("SELECT P.* FROM players as P WHERE Exists (SELECT O.Team as Teams FROM playerscontractoffers AS O WHERE O.Player=P.Number AND O.Type='RFA Offer Sheet Final') AND P.Retired=0 ORDER BY Overall desc" );
            ...
            <table  cellspacing="0" border="0" width="100%" class="tablesorter">
							...
							if($contractAccepted==0){
								if ($_SESSION['U_ID'] <> ""){
									if (!stristr($TeamOfferList,$_SESSION['U_ID']) && $contractAccepted==0){
										...
										//echo "<br />";
										echo $row_GetStatus['City'].' <a href="edit_restricted_offerSheet.php?player='.$row_GetSkaters['Number'].'&id='.$tmp_PR_ID.'&target=2&contract=final">'.$l_Edit.'</a> | <a href="remove_restricted_offerSheet.php?player='.$row_GetSkaters['Name'].'&id='.$tmp_PR_ID.'">'.$l_Remove.'</a>';
									}
								}
							}
                        ?>
                ...
                <?php 
					$query_GetGoalies = sprintf("SELECT G.* FROM goalies as G WHERE Exists (SELECT O.Team as Teams FROM playerscontractoffers AS O WHERE O.Player=G.Name AND O.Type='Goalie RFA Offer Sheet Final') AND G.Retired=0  ORDER BY Overall desc");
						...
						if($contractAccepted==0){
							if ($_SESSION['U_ID'] <> ""){
								if (!stristr($TeamOfferList,$_SESSION['U_ID']) && $contractAccepted==0){
									...
									echo $row_GetStatus['City'].' <a href="edit_offer_sheet.php?player='.$row_GetGoalies['Number'].'&id='.$tmp_PR_ID.'&type=goalie&target=2&contract=final">'.$l_Edit.'</a> | <a href="remove_rfa_offer_sheet.php?player='.$row_GetGoalies['Name'].'&type=goalie&id='.$tmp_PR_ID.'">'.$l_Remove.'</a>';
								}
							}
						}
                    ?>
