...
if ($ID_GetAction == 1){
	...
	$file = $uploaddir . $row_GetSeasons['Schedule'];
	if($_SESSION['current_FarmLeague'] == "True"){
		$updateGoTo = "import_csv_farmschedule.php?action=1";
	} else {
...
if($_SESSION['current_SeasonType']==0){
	$query_PlayoffRound = "SELECT Round FROM schedule WHERE Season_ID=".$_SESSION['current_SeasonID']." ORDER BY Round DESC LIMIT 0,1";
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
	...
	if ($row >> 0){
		...
		$TMP_GameNumber = $data[0];
			...
			$query_CheckPlayer = sprintf("SELECT Number FROM schedule WHERE Number=%s AND Season_ID=%s", $TMP_GameNumber, $ActiveSeason);
		...
		if ($SQLAction=="INSERT"){
			$insertSQL = sprintf("INSERT INTO schedule (Number,Day,Play,VisitorTeam,VisitorScore,HomeTeam,HomeScore,Overtime,ShootOut,Round,Season_ID) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
			...
			$updateSQL = sprintf("UPDATE schedule set Day=%s,Play=%s,VisitorTeam=%s,VisitorScore=%s,HomeTeam=%s,HomeScore=%s,Overtime=%s,ShootOut=%s WHERE Number=%s AND Season_ID=$ActiveSeason",
				GetSQLValueString($TMP_Day, "double"),
				GetSQLValueString($TMP_Play, "text"),
				GetSQLValueString($TMP_VisitorTeam,  "text"),
				GetSQLValueString($TMP_VisitorScore, "double"),
...
$updateSQL = "UPDATE config SET LastModifiedSchedule='".strftime('%Y-%m-%d %H:%M:%S', strtotime('now'))."'";
$Result1 = mysql_query($updateSQL, $connection) or die(mysql_error());
