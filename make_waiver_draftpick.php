...
$GetTeamID = "0";
if (isset($_GET['team'])) {
  $GetTeamID = (get_magic_quotes_gpc()) ? $_GET['team'] : addslashes($_GET['team']);
} 
$GetRound = "0";
if (isset($_GET['round'])) {
  $GetRound = (get_magic_quotes_gpc()) ? $_GET['round'] : addslashes($_GET['round']);
} 
$GetOverview = "0";
if (isset($_GET['overview'])) {
  $GetOverview = (get_magic_quotes_gpc()) ? $_GET['overview'] : addslashes($_GET['overview']);
} 
...
if (isset($_POST['draft'])) {
  ...
  $Result1 = mysql_query($updateSQL, $connection) or die(mysql_error());
	...
	$updateSQL = "UPDATE players SET Team=".$GetTeamID." WHERE Name='".$_POST['sel1']."'";
    $Result1 = mysql_query($updateSQL, $connection) or die(mysql_error());
	...
	$updateSQL = sprintf("UPDATE waiver_draftpicks SET DateCreated=%s WHERE D_ID=%s",
		GetSQLValueString($NewTime, "date"),
		GetSQLValueString($row_GetRounds['D_ID'], "int"));
  	$Result1 = mysql_query($updateSQL, $connection) or die(mysql_error());
  
	if ($totalRows_GetRounds > 0){
		$query_GetLast10 = sprintf("SELECT Name FROM waiver_draftpicks WHERE Name <> '' Order By DateCreated desc limit 0,10");
...
$query_GetDraftList = sprintf("SELECT DraftList FROM waiver_draftpicks WHERE D_ID=%s",$GetPickID);
...
$query_GetRoster = sprintf("SELECT Overall, Name, Number, PO, Age FROM players WHERE (Contract = 0 OR Team='' OR Team IS NULL) AND Retired=0 AND Suspension<99 AND Name NOT IN (SELECT Name FROM waiver_draftpicks WHERE Draft_ID=%s) %s UNION ALL SELECT Overall, Name, Number, PO, Age FROM goalies WHERE (Contract = 0 OR Team='' OR Team IS NULL) AND Retired=0 AND Suspension<99 AND Name NOT IN (SELECT Name FROM waiver_draftpicks WHERE Draft_ID=%s) %s ORDER BY Overall desc , Name asc", $GetDraftID, $extra_SQL, $GetDraftID, $extra_SQL);
...
<input type="hidden" name="team" value="<?php echo $GetTeamID; ?>" />
<input type="hidden" name="round" value="<?php echo $GetRound; ?>" />
<input type="hidden" name="overall" value="<?php echo $GetOverview; ?>" />
...
<tr>
  <td width="50%" valign="top">
	<select name="sel1" size="10" id="sel1" style="width:400px;"> 
		<?php 
		do { 			
			echo '<option value="'.$row_GetRoster['Name'].'">'.$row_GetRoster['Overall'].'. '.$row_GetRoster['Name'].'&nbsp;&nbsp;&nbsp;&nbsp;(age:'.$row_GetRoster['Age'].')</option>';
...
<tbody>
<form action="nba_waiver_draftpick.php" method="post" name="form" id="form">
<input type="hidden" name="id" value="<?php echo $GetPickID; ?>" />
<input type="hidden" name="draft" value="<?php echo $GetDraftID; ?>" />
<input type="hidden" name="team" value="<?php echo $GetTeamID; ?>" />
<input type="hidden" name="round" value="<?php echo $GetRound; ?>" />
<input type="hidden" name="overall" value="<?php echo $GetOverview; ?>" />
    ...
    <tr>
    	<td>Rating:</td>
        <td><select name="rating" size="1" id="rating" tabindex="5">
