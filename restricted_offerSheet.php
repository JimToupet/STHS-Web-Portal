...
case 'en': 
	...
	$AgentAskingPrice = "of current market value";
	...
	$SummaryText6 = 'Your roster of <strong>%item1</strong> is over the league limit of <strong>%item2</strong> <br><br>The league will not approve this deal until you free up some roster spots on your team.';
...
case 'fr': 
	...
	$SummaryText6 = 'Votre r&ocirc;le de %item1 est au-dessus de la limite de ligue de %item2 que la ligue n&apos;approuvera pas cette affaire jusqu&apos;&agrave; ce que vous lib&eacute;riez vers le haut quelques taches de r&ocirc;le sur votre &eacute;quipe.';
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form")) {
	...
	if($_POST["position"] == 5){
		$offerType="Goalie RFA Offer Sheet";
	} else {
		$offerType="RFA Offer Sheet";
	}
	
	$insertSQL = sprintf("INSERT INTO playerscontractoffers (Player,Type,Team,DateCreated,Contract,Salary1,Salary2,Salary3,Salary4,Salary5,Salary6,Salary7,Salary8,Salary9,Salary10,NoTrade,Bonus,NoTradeSalary,ContractType,Compensation,Approved) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'False')",
            GetSQLValueString(ParseSQL($_POST['player']), "text"),
            GetSQLValueString($offerType, "text"),
...
$RosterLimit=$row_GetWebInfo['RosterLimit'];
...
$updatedTxtItems = array(number_format($SalaryCap - $CurrentPayroll,0), number_format($CurrentOffers,0), number_format($SalaryCap - $CurrentPayroll - $CurrentOffers,0));
...
$updatedTxtItems = array(getTeamRoster($_SESSION['U_ID'],$connection), $RosterLimit);
$SummaryText6 = str_replace($tmpTxtItems, $updatedTxtItems, $SummaryText6);
...
<title><?php echo $l_Offer; ?> - <?php echo $_SESSION['SiteName'] ; ?></title>
...
var bargain = 1;
var salaryCapRemaining = <?php echo (($SalaryCap * 1.05) - $CurrentPayroll - $CurrentOffers); ?>;
...
var ExtraBonus = 0;
var TeamRosterCount = <?php echo getTeamRoster($_SESSION['U_ID'],$connection);?>;
$('#UFAText').hide();
$('#NoTrade').hide();
$('#ExpectedText').hide();	
$('#BonusText').hide();	
  ...
  if(document.contract_form.yearsoff.value==1){
	...
	function updateOffer() {
		...
		var YEAR_1 = "<?php echo ($_SESSION['current_Season'])."-".substr($_SESSION['current_Season']+1, -2);?>";
		var YEAR_2 = "<?php echo ($_SESSION['current_Season']+1)."-".substr($_SESSION['current_Season']+2, -2);?>";
		var YEAR_3 = "<?php echo ($_SESSION['current_Season']+2)."-".substr($_SESSION['current_Season']+3, -2);?>";
		var YEAR_4 = "<?php echo ($_SESSION['current_Season']+3)."-".substr($_SESSION['current_Season']+4, -2);?>";
		var YEAR_5 = "<?php echo ($_SESSION['current_Season']+4)."-".substr($_SESSION['current_Season']+5, -2);?>";
		var YEAR_6 = "<?php echo ($_SESSION['current_Season']+5)."-".substr($_SESSION['current_Season']+6, -2);?>";
		var YEAR_7 = "<?php echo ($_SESSION['current_Season']+6)."-".substr($_SESSION['current_Season']+7, -2);?>";
		var YEAR_8 = "<?php echo ($_SESSION['current_Season']+7)."-".substr($_SESSION['current_Season']+8, -2);?>";
		var YEAR_9 = "<?php echo ($_SESSION['current_Season']+8)."-".substr($_SESSION['current_Season']+9, -2);?>";
		var YEAR_10 = "<?php echo ($_SESSION['current_Season']+9)."-".substr($_SESSION['current_Season']+10, -2);?>";
		...
		if(salaryCapRemaining - YEAR_1_SALARY_DEMAND < 0){
			if(YEAR_1_SALARY_DEMAND > <?php echo $WaiverSalaryExemption;?>){
				...
				if(TeamRosterCount > <?php echo $RosterLimit;?>){
					$("#buttonEmail").hide();
					$('#ownerApproval').html('<?php echo $SummaryText6;?>');
					$('#ownerApproval').css({"background-color":"red","color":"white"});
				} else {
					$("#buttonEmail").show();	
					$('#ownerApproval').html('<?php echo $SummaryText4;?>');
...
<td id="PlayerInfo" style="background-color:#ededed; padding:0px 6px 6px 6px;vertical-align:top;">
    ...
    <tr>
        <td colspan="3" style="vertical-align:top;"><strong style="font-size:1.6em; text-transform:uppercase;"><?php echo $l_Offer." : ".$row_GetPlayer['Name']; ?></strong>
...
<div id="loader">
	<div id="tabs" class="fieldsetleft">
 	<h3 style="border-bottom-width:1px; border-bottom-style:dotted; border-bottom-color:#999; padding-bottom:5px; margin-bottom:10px;"><?php echo $ContractOptions;?></h3>
        ...
        <option value="6" selected="selected">100% <?php echo $AgentAskingPrice;?>.</option>
        <option value="5">90% <?php echo $AgentAskingPrice;?>.</option>
        <option value="4">80% <?php echo $AgentAskingPrice;?>.</option>
        <option value="3">70% <?php echo $AgentAskingPrice;?>.</option>
        <option value="2">60% <?php echo $AgentAskingPrice;?>.</option>
        <option value="1">50% <?php echo $AgentAskingPrice;?>.</option>
        ...
        <option value="12">175% <?php echo $AgentAskingPrice;?>.</option>
        <option value="13">200% <?php echo $AgentAskingPrice;?>.</option>
        </select>
        </div>
