    ...
    <h1><?php echo $l_nav_unrestricted_free_agents;?></h1> 
       <br /><br clear="all" />
        ...
        <ul>
              ...
              <li id="selected"><a href="ufa-free-agents_signed.php"><?php echo $l_tab4; ?></a></li>
        </ul>
		
        <div id="tab-content">
            <?php
			 	$query_GetSkatersSigned = sprintf("SELECT P.Name, P.Number, O.*, T.Abbre FROM playerscontractoffers AS O, proteam as T, players as P WHERE O.Player=P.Number AND O.Team=T.Number AND O.Type='Signed' AND Not Exists (SELECT 1 FROM playersextensionoffers AS E WHERE E.Player=P.Number AND E.PlayerType <> 'goalie') AND P.Retired=0 ORDER BY T.City, O.DateCreated");
              ...
              <table cellspacing="0" border="0" width="100%" class="tablesorter">
                ...
                <tr>
                    ...
                    if ($_SESSION['U_Admin']==1){ 
                        echo '<a href="remove_offer_sheet.php?player='.$row_GetSkatersSigned['Player'].'&id='.$row_GetSkatersSigned['PR_ID'].'">'.$l_Remove.'</a>';
                    } else {
                ...
                <tfoot> 
                <tr>
                	<td colspan="14" align="right"><a href="javascript:confirmDelete('remove_all_offer_sheet.php');"><strong><?php echo $l_RemoveAll;?></strong></a></td>
                  </tr>
              	</tfoot>
                <?php  } ?>
                </table>
                ...
                <?php
			 	$query_GetGoaliesSigned = sprintf("SELECT G.Name, G.Number, O.*, T.Abbre FROM playerscontractoffers AS O, proteam as T, goalies as G WHERE O.Player=G.Number AND O.Team=T.Number AND O.Type='Goalie Signed' AND Not Exists (SELECT 1 FROM playersextensionoffers AS E WHERE E.Player=G.Number AND E.PlayerType = 'goalie') AND G.Retired=0 ORDER BY T.City, O.DateCreated");
              ...
              <table cellspacing="0" border="0" width="100%" class="tablesorter">
                ...
                <tr>
                    ...
                    <?php 
                    if ($_SESSION['U_Admin']==1){ 
                        echo '<a href="remove_offer_sheet.php?player='.$row_GetGoaliesSigned['Player'].'&id='.$row_GetGoaliesSigned['PR_ID'].'">'.$l_Remove.'</a>';
                    } else {
