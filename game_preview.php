...
$query_GetGame = sprintf("SELECT * FROM schedule WHERE S_ID=%s", $Game_ID);
...
$query_GetHT = sprintf("SELECT * FROM proteam as p, proteamstandings as s WHERE p.Number=s.Number AND p.Number=%s AND s.Season_ID=%s", $row_GetGame['HomeTeam'], $_SESSION['current_SeasonID']);
$GetHT = mysql_query($query_GetHT, $connection) or die(mysql_error());
$row_GetHT = mysql_fetch_assoc($GetHT);
$HomeTeam=$row_GetHT['Name'];

$query_GetHTRoster = sprintf("SELECT p.*, SUM(s.ProGP) as ProGP, SUM(s.ProG) as ProG, SUM(s.ProA) AS ProA, SUM(s.ProPoint) as ProPoint, SUM(s.ProPim) as ProPim, SUM(s.ProPlusMinus) as ProPlusMinus FROM players as p, playerstats as s WHERE p.Name=s.Name AND p.Team=%s AND s.Season_ID=%s AND p.Status0 > 1 GROUP BY s.Name ORDER BY ProPoint DESC, ProG DESC, ProGP asc", $row_GetGame['HomeTeam'], $SID_GetTop5);
$GetHTRoster = mysql_query($query_GetHTRoster, $connection) or die(mysql_error());
$row_GetHTRoster = mysql_fetch_assoc($GetHTRoster);

$query_GetHTGoalies = sprintf("SELECT g.*, SUM(s.ProGP) as ProGP, SUM(s.ProW) as ProW, SUM(s.ProL) AS ProL, SUM(s.ProMinPlay) as ProMinPlay, SUM(s.ProGA) as ProGA, SUM(s.ProSA) as ProSA, SUM(s.ProShutouts) as ProShutouts, SUM(ProOTL) as ProOTL FROM goalies as g, goaliestats as s WHERE g.Name=s.Name AND g.Team=%s AND s.Season_ID=%s AND g.Status1 > 1 GROUP BY s.Name ORDER BY ProW DESC, ProMinPlay DESC, ProGP asc", $row_GetGame['HomeTeam'], $SID_GetTop5);
$GetHTGoalies = mysql_query($query_GetHTGoalies, $connection) or die(mysql_error());
$row_GetHTGoalies = mysql_fetch_assoc($GetHTGoalies);

$query_GetVT = sprintf("SELECT * FROM proteam as p, proteamstandings as s WHERE p.Number=s.Number AND p.Number=%s AND s.Season_ID=%s", $row_GetGame['VisitorTeam'], $_SESSION['current_SeasonID']);
$GetVT = mysql_query($query_GetVT, $connection) or die(mysql_error());
$row_GetVT = mysql_fetch_assoc($GetVT);
$VisitorTeam=$row_GetVT['Name'];

$query_GetVTRoster = sprintf("SELECT p.*, SUM(s.ProGP) as ProGP, SUM(s.ProG) as ProG, SUM(s.ProA) AS ProA, SUM(s.ProPoint) as ProPoint, SUM(s.ProPim) as ProPim, SUM(s.ProPlusMinus) as ProPlusMinus FROM players as p, playerstats as s WHERE p.Name=s.Name AND p.Team=%s AND s.Season_ID=%s AND p.Status0 > 1 GROUP BY s.Name ORDER BY ProPoint DESC, ProG DESC, ProGP asc", $row_GetGame['VisitorTeam'], $SID_GetTop5);
$GetVTRoster = mysql_query($query_GetVTRoster, $connection) or die(mysql_error());
$row_GetVTRoster = mysql_fetch_assoc($GetVTRoster);

$query_GetVTGoalies = sprintf("SELECT g.*, SUM(s.ProGP) as ProGP, SUM(s.ProW) as ProW, SUM(s.ProL) AS ProL, SUM(s.ProMinPlay) as ProMinPlay, SUM(s.ProGA) as ProGA, SUM(s.ProSA) as ProSA, SUM(s.ProShutouts) as ProShutouts, SUM(ProOTL) as ProOTL FROM goalies as g, goaliestats as s WHERE g.Name=s.Name AND g.Team=%s AND s.Season_ID=%s AND g.Status1 > 1 GROUP BY s.Name ORDER BY ProW DESC, ProMinPlay DESC, ProGP asc", $row_GetGame['VisitorTeam'], $SID_GetTop5);
$GetVTGoalies = mysql_query($query_GetVTGoalies, $connection) or die(mysql_error());
$row_GetVTGoalies = mysql_fetch_assoc($GetVTGoalies);

$query_GetRecord = sprintf("SELECT * FROM schedule as p, seasons as s WHERE Season = '%s' AND p.Season_ID=s.S_ID AND p.VisitorTeam=%s AND p.HomeTeam=%s AND s.SeasonType < 2 UNION ALL SELECT * FROM schedule as p, seasons as s WHERE Season = '%s' AND p.Season_ID=s.S_ID AND p.HomeTeam=%s AND p.VisitorTeam=%s AND s.SeasonType < 2 ORDER BY Season desc, SeasonType DESC, Day asc", $_SESSION['current_Season'], $row_GetGame['VisitorTeam'], $row_GetGame['HomeTeam'], $_SESSION['current_Season'], $row_GetGame['VisitorTeam'], $row_GetGame['HomeTeam']);
...
<title>Game <?php echo $row_GetGame['Number']." Preview: ".$row_GetVT['City']." at ".$row_GetHT['City'];?> - <?php echo $_SESSION['SiteName'] ; ?></title>
...
<article>
    ...
    <section>
    	<h1>Game <?php echo $row_GetGame['Number']." Preview: ".$row_GetVT['City']." at ".$row_GetHT['City'];?></h1>
        <br clear="all" />
...
do{
	...
	$query_GetHT2 = sprintf("SELECT LogoTiny, City, Name FROM proteam WHERE Number=%s", $row_GetRecord['HomeTeam']);
	$GetHT2 = mysql_query($query_GetHT2, $connection) or die(mysql_error());
	$row_GetHT2 = mysql_fetch_assoc($GetHT2);
	$HomeTeam=$row_GetHT2['City']." ".$row_GetHT2['Name'];
	
	$query_GetVT2 = sprintf("SELECT LogoTiny, City, Name FROM proteam WHERE Number=%s", $row_GetRecord['VisitorTeam']);
	$GetVT2 = mysql_query($query_GetVT2, $connection) or die(mysql_error());
	$row_GetVT2 = mysql_fetch_assoc($GetVT2);
	$VisitorTeam=$row_GetVT2['City']." ".$row_GetVT2['Name'];

	$query_GetLink = sprintf("SELECT todaysgame.Link FROM todaysgame WHERE todaysgame.GameNumber=CONCAT('Pro',CAST(".$row_GetRecord['Number']." as CHAR)) AND todaysgame.Season_ID=%s", $row_GetRecord['S_ID']);
	...
	$GameLink = $row_GetLink['Link'];
		...
		if($GameLink != ""){
			...
			$query_GetLink = sprintf("SELECT Link FROM todaysgame WHERE Season_ID=%s AND Type='Pro'ORDER BY Link DESC limit 0,1", $row_GetRecord['S_ID']);
			$GetLink = mysql_query($query_GetLink, $connection) or die(mysql_error());
			$row_GetLink = mysql_fetch_assoc($GetLink);
...
?>
        ...
        <?php if($row_GetVT['JerseyAway'] != ""){
        	...
        	echo "<img src='".$_SESSION['DomainName']."/image/logos/huge/".$row_GetVT['LogoLarge']."' vspace='6' id='TeamPhoto1' border=0>";
		}
		...
		echo "<td align=center>".$row_GetVTRoster['ProGP']."</td>";
		echo "<td align=center>".$row_GetVTRoster['ProG']."</td>";
		echo "<td align=center>".$row_GetVTRoster['ProA']."</td>";
		echo "<td align=center>".$row_GetVTRoster['ProPoint']."</td>";
		echo "<td align=center>".$row_GetVTRoster['ProPlusMinus']."</td>";
		echo "<td align=center>".$row_GetVTRoster['ProPim']."</td>";
		echo "</tr>";
		}
		...
		echo "<tr>";
		echo "<td><a href='goalie.php?player=".$row_GetVTGoalies['Name']."'>".$row_GetVTGoalies['Name']."</a></td>";
		echo "<td align=center>".$row_GetVTGoalies['CON']."</td>";
		echo '<td align="center">'.$row_GetVTGoalies['ProGP'].'</td>';
        echo '<td align="center">'.$row_GetVTGoalies['ProW'].'</td>';
        echo '<td align="center">'.$row_GetVTGoalies['ProL'].'</td>';
        echo '<td align="center">'.$row_GetVTGoalies['ProOTL'].'</td>';
        if ($row_GetVTGoalies['ProMinPlay'] > 0){
			echo '<td align="center">'.number_format(($row_GetVTGoalies['ProGA'] / minutes($row_GetVTGoalies['ProMinPlay']))*60,2).'</td>';
		} else { 
			echo '<td align="center">0</td>'; 
		}
        if ($row_GetVTGoalies['ProMinPlay'] > 0){ 
			echo '<td align="center">'.number_format($row_GetVTGoalies['ProSA'] / ($row_GetVTGoalies['ProGA'] + $row_GetVTGoalies['ProSA']),3).'</td>';
		} else { 
			echo '<td align="center">0</td>'; 
		}
        echo '<td align="center">'.$row_GetVTGoalies['ProShutouts'].'</td>';
		echo "</tr>";
		}
		...
		echo "<td align=center>".$row_GetHTRoster['ProGP']."</td>";
		echo "<td align=center>".$row_GetHTRoster['ProG']."</td>";
		echo "<td align=center>".$row_GetHTRoster['ProA']."</td>";
		echo "<td align=center>".$row_GetHTRoster['ProPoint']."</td>";
		echo "<td align=center>".$row_GetHTRoster['ProPlusMinus']."</td>";
		echo "<td align=center>".$row_GetHTRoster['ProPim']."</td>";
		echo "</tr>";
		}
		...
		echo "<tr>";
		echo "<td><a href='goalie.php?player=".$row_GetHTGoalies['Name']."'>".$row_GetHTGoalies['Name']."</a></td>";
		echo "<td align=center>".$row_GetHTGoalies['CON']."</td>";
		echo '<td align="center">'.$row_GetHTGoalies['ProGP'].'</td>';
        echo '<td align="center">'.$row_GetHTGoalies['ProW'].'</td>';
        echo '<td align="center">'.$row_GetHTGoalies['ProL'].'</td>';
        echo '<td align="center">'.$row_GetHTGoalies['ProOTL'].'</td>';
        if ($row_GetHTGoalies['ProMinPlay'] > 0){
			echo '<td align="center">'.number_format(($row_GetHTGoalies['ProGA'] / minutes($row_GetHTGoalies['ProMinPlay']))*60,2).'</td>';
		} else { 
			echo '<td align="center">0</td>'; 
		}
        if ($row_GetHTGoalies['ProMinPlay'] > 0){ 
			echo '<td align="center">'.number_format($row_GetHTGoalies['ProSA'] / ($row_GetHTGoalies['ProGA'] + $row_GetHTGoalies['ProSA']),3).'</td>';
		} else { 
			echo '<td align="center">0</td>'; 
		}
        echo '<td align="center">'.$row_GetHTGoalies['ProShutouts'].'</td>';
		echo "</tr>";
		}
