...
if ($_SESSION['current_Team_ID']==0) {
	...
	$query_GetSchedule = sprintf("SELECT schedule.* FROM schedule  WHERE (schedule.VisitorTeam='%s' OR schedule.HomeTeam='%s') AND schedule.Season_ID=%s ORDER BY schedule.Number", $TID_GetSchedule, $TID_GetSchedule, $SID_GetSchedule);
}
...
<article>
   ...
   <div style="float:left; padding-bottom:2px"> <h1><?php echo $l_nav_schedules;?></h1></h1></div>
        ...
        <form action="pro_schedule.php" method="post" name="form1">
    ...
    <tr>
		...
		<th><?php echo $l_Game;?></th>
		<th><?php echo $l_VisitorTeam;?></th>
		<th><?php echo $l_Score;?></th>
		<th><?php echo $l_HomeTeam;?></th>
		<th><?php echo $l_Score;?></th>
		<th><?php echo $l_Overtime;?></th>
		<th><?php echo $l_ShootOut;?></th>
	...
	if($totalRows_GetSchedule > 0){
		do { 
			...
			$query_GetLink = sprintf("SELECT todaysgame.Link FROM todaysgame WHERE todaysgame.GameNumber=CONCAT('Pro',CAST(".$row_GetSchedule['Number']." as CHAR)) AND todaysgame.Season_ID=%s", $SID_GetSchedule);
    ...
    <tr>
		...
		<td align="center"><?php if($_SESSION['current_Team_ID'] == $row_GetSchedule['VisitorTeam']){echo $row_GetVisitorTeam['City']." ".$row_GetVisitorTeam['Name']; } else { echo "<a href='pro_roster.php?team=".$row_GetSchedule['VisitorTeam']."'>".$row_GetVisitorTeam['City']." ".$row_GetVisitorTeam['Name']."</a>"; } ?></td>
		<td align="center"><?php echo $row_GetSchedule['VisitorScore']; ?></td>
		<td align="center"><?php if($_SESSION['current_Team_ID'] == $row_GetSchedule['HomeTeam']){echo $row_GetHomeTeam['City']." ".$row_GetHomeTeam['Name']; } else { echo "<a href='pro_roster.php?team=".$row_GetSchedule['HomeTeam']."'>".$row_GetHomeTeam['City']." ".$row_GetHomeTeam['Name']."</a>"; } ?></td>
