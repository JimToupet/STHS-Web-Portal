...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	$insertSQL = sprintf("update draft set DraftName=%s,DraftPickTime=%s,Year=%s,Season_ID=%s,LotteryWinner=%s WHERE D_ID=%s",
...
$query_GetDraftPicks = sprintf("SELECT * FROM draft WHERE D_ID=%s",$ID_GetID);
...
<select name="LotteryWinner" size="1"><option value="0"><?php echo $l_None; ?></option>
    <?php do { ?>
    <option value="<?php echo $row_GetTeam2['Number']; ?>"  <?php if ($row_GetDraftPicks['LotteryWinner']==$row_GetTeam2['Number']){ echo 'selected'; };?>><?php echo $row_GetTeam2['City']." ".$row_GetTeam2['Name']; ?></option>
...
$query_CheckDraftPicks = sprintf("SELECT * FROM draft WHERE Year=%s",$row_GetDraftPicks['Year']);
...
$totalRows_CheckDraftPicks = mysql_num_rows($CheckDraftPicks);
   	do { 
	$tmpSelected = "";
	if ($row_CheckDraftPicks['Year']==$row_GetDraftYear['Year']){ $tmpSelected = 'selected'; };
		echo '<option value="'.$row_GetDraftYear['Year'].'" '.$tmpSelected.'>Year '.$row_GetDraftYear['Year'].'</option>';
