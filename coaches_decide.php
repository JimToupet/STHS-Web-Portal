...
<article>
    ...
    <section>
        ...
        <ul>
              ...
              <li id="selected"><a href="coaches_decide.php"><?php echo $l_tab3; ?></a></li>
        ...
        <div id="tab-content">
			<?php 
				...
				$query_GetCoaches = sprintf("SELECT P.* FROM coaches as P WHERE Exists (SELECT O.Team as Teams FROM playerscontractoffers AS O WHERE O.Player=P.Number AND O.Type='Coach Offer Sheet Final') ORDER BY LD desc" );
		...
		$contractAccepted=0;
			do { 
				...
				$query_GetStatus= sprintf("SELECT O.Type, O.PR_ID, O.Team, T.Abbre, T.Name FROM playerscontractoffers AS O, proteam AS T WHERE O.Player='%s' AND O.Team=T.Number Order By T.Name asc",addslashes($row_GetCoaches['Number']) );
		  ...
		  <tr id="FreeAgent_<?php echo $rowCount;?>" >
			...
			<td style="text-align:center" valign="top">
							...
							if($contractAccepted==0){
								if ($_SESSION['U_ID'] <> ""){
									if (!stristr($TeamOfferList,$_SESSION['U_ID']) && $contractAccepted==0){
										...
										echo "<br />";
										echo $row_GetStatus['City'].' <a href="edit_coach_offer_sheet.php?coach='.$row_GetCoaches['Number'].'&id='.$tmp_PR_ID.'&target=2&contract=final">'.$l_Edit.'</a> | <a href="remove_coach_offer_sheet.php?coach='.$row_GetCoaches['Number'].'&id='.$tmp_PR_ID.'">'.$l_Remove.'</a>';
									}
								}
							}
                        ?>
						...
						if($contractAccepted==1 && $_SESSION['U_Admin']==1){
							echo '<a href="#" id="'.$rowCount.'"  player="'.$row_GetCoaches['Number'].'" type="coach" class="choose_offer">'.$l_Sign.'</a>';
						} else {
