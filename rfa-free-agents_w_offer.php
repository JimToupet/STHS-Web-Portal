...
$(function(){ 
  ...
  $('#cssdropdown li.headlink').hover(
	...
	$(".choose_offer").live('click', function()
	{
		...
		$.ajax({
			type: "POST",
			url: "choose_final_3_rfa_offers.php",
...
<article>
    ...
    <br /><br clear="all" />
        ...
        <ul>
              ...
              <li id="selected"><a href="rfa-free-agents_w_offer.php"><?php echo $l_tab2; ?></a></li>
        ...
        <div id="tab-content">
           <?php 
				...
				$query_GetSkaters = sprintf("SELECT P.* FROM players as P WHERE Exists (SELECT O.Team as Teams FROM playerscontractoffers AS O WHERE O.Player=P.Number AND O.Type='RFA Offer Sheet')  AND P.Retired=0 ORDER BY Overall desc" );
            ...
            <table  cellspacing="0" border="0" width="100%" class="tablesorter">
				...
				$tmpRowCount=0;
                    do { 
						...
						$query_GetStatus= sprintf("SELECT O.Type, O.DateCreated, O.PR_ID, O.Team, T.Abbre FROM playerscontractoffers AS O, proteam AS T WHERE O.Player=%s AND O.Team=T.Number Order By T.City asc",$row_GetSkaters['Number'] );
					...
					<?php 
                        ...
                        if($contractAccepted==0 && $_SESSION['U_ID'] <> ""){
							...
							if ($tmp_PlacedBid == 0 && $contractAccepted==0){
								echo '<a href="restricted_offerSheet.php?player='.$row_GetSkaters['Number'].'&type=player">'.$l_Send.'</a>';
							} else {
								echo $row_GetStatus['City'].' <a href="edit_restricted_offerSheet.php?player='.$row_GetSkaters['Number'].'&type=player&target=1&id='.$tmp_PR_ID.'">'.$l_Edit.'</a> | <a href="remove_rfa_offer_sheet.php?player='.$row_GetSkaters['Number'].'&id='.$tmp_PR_ID.'">'.$l_Remove.'</a>';
							}
						}
                        ?>
                        </td>
                        ...
                        if($contractAccepted==1 && $_SESSION['U_Admin']==1){
							echo '<a href="#" id="'.$rowCount.'" player="'.$row_GetSkaters['Number'].'" type="player" class="choose_offer">'.$l_Choose.'</a>';
                        } else {
                ...
                <?php 
					$query_GetGoalies = sprintf("SELECT G.* FROM goalies as G WHERE Exists (SELECT O.Team as Teams FROM playerscontractoffers AS O WHERE O.Player=G.Number AND O.Type='Goalie RFA Offer Sheet') AND G.Age < ".$UFA." AND G.Retired=0 ORDER BY Overall desc");
                  ...
                  <tr id="FreeAgent_<?php echo $rowCount;?>">
					...
					<?php 
						...
						if($contractAccepted==0 && $_SESSION['U_ID'] <> ""){
							...
							if ($tmp_PlacedBid == 0 && $contractAccepted==0){
								echo '<a href="restricted_offerSheet.php?player='.$row_GetGoalies['Number'].'&type=goalie">'.$l_Send.'</a>';
							} else {
								echo $row_GetStatus['City'].' <a href="edit_restricted_offerSheet.php?player='.$row_GetGoalies['Number'].'&id='.$tmp_PR_ID.'&target=1&type=goalie">'.$l_Edit.'</a> | <a href="remove_rfa_offer_sheet.php?player='.$row_GetGoalies['Number'].'&id='.$tmp_PR_ID.'&type=goalie">'.$l_Remove.'</a>';
							}
						}
                        ?>
                    </td>
                    <td align="right" valign="top" nowrap="nowrap">
						<?php 
                        if($contractAccepted==1 && $_SESSION['U_Admin']==1){
							echo '<a href="#" id="'.$rowCount.'" player="'.$row_GetGoalies['Number'].'" type="goalie" class="choose_offer">'.$l_Choose.'</a>';
							echo '<a href="#" id="'.$rowCount.'" player="'.$row_GetSkaters['Number'].'" type="player" class="choose_offer">'.$l_Choose.'</a>';
                        } else {
							$myDate1 = strftime('%d', strtotime('now'));
							$myDate2 = date("d", $timeStamp2);
